...
这些分组是基于文件名的发现入口。模块名不证明路由、DOM 选择器、实时窗口或功能启用。请在文件探索器中查看导入与片段。
...
| 令牌 | 出现次数 | 运行时对应 |
| --- | --- | --- |
| `hotkeyWindow` | 63 | 未按令牌验证 |
| `appView` | 18 | 未按令牌验证 |
| `annotationView` | 15 | 未按令牌验证 |
| `applyHotkeyWindow` | 10 | 未按令牌验证 |
| `editorWindow` | 8 | 未按令牌验证 |
| `browserCommentPopupWindow` | 5 | 未按令牌验证 |
| `activeConversationByWindow` | 4 | 未按令牌验证 |
| `browserView` | 1 | 未按令牌验证 |

## 网络声明

下列值声明于渲染层 CSP。许可与实际流量是不同事实。
...
来自独立换肤项目的选择器不属于此模型。工作台不会向应用注入代码。静态发现仅通过显式分析映射关联到归档截图。

[截图 GUI](codex-desktop-gui-map-evidence_Zh.md) · [DeepWiki](../site/gui-map.html)
