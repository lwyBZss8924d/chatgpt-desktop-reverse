#!/usr/bin/env node
/**
 * sweep5.mjs — start a fresh thread so the thread-scoped chrome exists, then
 * open each right-hand panel with its own shortcut and record the result.
 *
 * Panels are only mounted inside a conversation; every attempt on the Security
 * or Settings surfaces silently did nothing because the chrome was absent.
 */
...
fs.writeFileSync(path.join(OUT, "sweep5-results.json"), JSON.stringify({ pid: PID, at: new Date().toISOString(), results }, null, 2));
